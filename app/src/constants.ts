export const autoDisableRemoteLoggingIntervalInMinutes = 60
export const surveyMonkeyUrl = 'https://www.surveymonkey.com/r/7BMHJL8'
export const surveyMonkeyExitUrl = 'https://www.surveymonkey.com/survey-thanks'
export const hitSlop = { top: 44, bottom: 44, left: 44, right: 44 }
interface AttestationRestrictionEnvironment {
  credDefIDs: readonly string[]
  invitationUrl: string
}

export const AttestationRestrictions: { [key: string]: AttestationRestrictionEnvironment } = {
  Development: {
    credDefIDs: ['NXp6XcGeCR2MviWuY51Dva:3:CL:33557:asmlwallet', 'NXp6XcGeCR2MviWuY51Dva:3:CL:33557:asmlwallet_dev_v2'],
    invitationUrl:
      'https://traction-acapy-dev.apps.silver.devops.gov.bc.ca?c_i=',
  },
  Test: {
    credDefIDs: ['RycQpZ9b4NaXuT5ZGjXkUE:3:CL:120:asmlwallet', 'RycQpZ9b4NaXuT5ZGjXkUE:3:CL:120:asmlwallet_test_v2'],
    invitationUrl:
      'https://traction-acapy-test.apps.silver.devops.gov.bc.ca?c_i=',
  },
  Production: {
    credDefIDs: ['XqaRXJt4sXE6TRpfGpVbGw:3:CL:655:asmlwallet'],
    invitationUrl:
      'https://traction-acapy-prod.apps.silver.devops.gov.bc.ca?c_i=',
  },
} as const

export const appleAppStoreUrl = 'https://apps.apple.com/ca/app/keyring-wallet/id1587380443'
export const googlePlayStoreUrl = 'https://play.google.com/store/apps/details?id=ca.keyring.wallet&hl=en-CA'
export const appHelpUrl = 'https://keyring.app/help'

export const KeyRingThemeNames = {
  KeyRing: 'keyring',
} as const
