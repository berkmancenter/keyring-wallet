...
| [editable-multi-profile-plan.md](./editable-multi-profile-plan.md) | Making a user's R-Card profile editable after onboarding (Phase 1), then letting a user hold multiple profiles and choose which is active when connecting (Phase 2). Reserves, but does not build, a link to the separate and not-yet-decided VTA "persona" concept from `keyring-on-the-vta-farm.md` | Proposal for review. No code written |
