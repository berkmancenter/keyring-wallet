...
| [keyring-on-the-vta-farm.md](./keyring-on-the-vta-farm.md) | Keyring as a full participant at every level of the ecosystem's documented developer path, against a **Farm-hosted** VTA rather than one we run: provisioning from the phone, persona and membership, and the join request qualified by Keyring-minted witnessed VRCs. Carries the React Native portability contributions the path forces | Proposed. Blocked on nothing; opens with a pin advance and a Farm measurement |
