...
| [`2026-09-06-agent.md`](./reference-app-sdk-packaging/2026-09-06-agent.md) | agent | R5 as built: the open `TrustTaskRegistry` (engine-side dispatch) and `ITrustTaskDisplayRegistry` (UI-side, mirroring `ICredentialDisplayRegistry`) behind one `registerTrustTask({ spec, orchestration, renderer })` call; `ceremony.ts`'s VRC/witness/discovery dispatch recast as the registry's first four registrations, preserving all existing behaviour (full pre-existing test suite unchanged and passing); the generic render/approve-deny scaffold (`genericApproval.ts`, `TrustTaskApprovalCard.tsx`); the Approver demo profile's concrete scenario and Type URI (`approver/access-request/0.1` under `keyring.appliedtechnologylab.org`); the DI-token-collision check between trading-card and approver (none found) and the `ACTIVE_DEMO_PROFILE` picker built anyway per requirement D. Unit tests, typecheck and lint are clean. Records the on-device e2e round trip as not yet passing at the time it was written — see [`2026-09-06-agent2.md`](./reference-app-sdk-packaging/2026-09-06-agent2.md) for the resolution |
| [`2026-09-06-agent2.md`](./reference-app-sdk-packaging/2026-09-06-agent2.md) | agent | Resolves the on-device gap `2026-09-06-agent.md` left open. One real, independently-confirmed bug found and fixed along the way (`ListCredentials.tsx`'s `ListFooterComponent` passed as an inline function, causing spurious footer-subtree remounts) — did not turn out to be this symptom's cause. The actual cause: bifold's onboarding-tour overlay (`TourOverlay.tsx`), which auto-starts the first time the Wallet tab renders with zero credentials and swallows every tap regardless of the accessibility tree underneath, with an existing but uncalled guard (`dismissTourIfPresent`) elsewhere in `e2e/lib/flows.js`. Fixed in the e2e script, not the app. Verified with a real two-emulator run: full round trip passes — request sent, `TrustTaskApprovalCard` rendered, approved, signed decision received |
| [`2026-09-10-bm.md`](./reference-app-sdk-packaging/2026-09-10-bm.md) | BM | The Approver's UI recast off a Wallet-tab-wide banner onto each contact's own details screen (`ApproverContactSection` + `ApproverGlobalListener`), riding two new general-purpose container tokens (`COMPONENT_CONTACT_DETAILS_FOOTER`, `COMPONENT_APP_GLOBAL_LISTENER`) alongside the slightly earlier `COMPONENT_CONTACT_CARD`; three bugs the move surfaced (late-`connectionId` sync, DIDComm label vs. VRC/RCard name, free-text field overflow) and their fixes; `ACTIVE_DEMO_PROFILE=none` closing from runtime-inert to true build-time bundle exclusion via Metro, finishing R4's gap; and a round of dev-mode module-resolution fixes (Android Gradle namespace/fileTree patches, `tsc`/Jest resolution of `@bifold/react-hooks` and `@bifold/credo-tsp-adapter` to source, a missing `react-hooks` re-export) that were silently blocking the walkthrough R1/R2 already promise works |
...
|---|---|---|---|
| 1 | The Approver | ✅ built | `TrustTaskRegistry` (open `typeUri` → `{spec, orchestration, renderer}` registration, replacing `ceremony.ts`'s closed if/else) and `ITrustTaskDisplayRegistry` (mirroring `ICredentialDisplayRegistry`) are built and unit-tested; a concrete demo profile (`app/src/demo-profiles/approver/`) rides both, with a real signed approve/deny round trip confirmed on two Android emulators. The UI now lives on each contact's own Contact Details screen (`ApproverContactSection`, via `COMPONENT_CONTACT_DETAILS_FOOTER`) rather than a Wallet-tab-wide banner, with an app-wide toast listener (`ApproverGlobalListener`, via `COMPONENT_APP_GLOBAL_LISTENER`) as the entry point from elsewhere in the app. See [`2026-09-06-agent.md`](./reference-app-sdk-packaging/2026-09-06-agent.md), [`2026-09-06-agent2.md`](./reference-app-sdk-packaging/2026-09-06-agent2.md) and [`2026-09-10-bm.md`](./reference-app-sdk-packaging/2026-09-10-bm.md) |
...
- **R4 — a scaffolding convention** — a `demo-profiles/`-style directory sibling to `keyring-theme/` (the existing, currently-unbuilt `app/src/workflow-architecture-example/` is the closest precedent) — so "add a new use case" has one obvious place for a theme + container overrides + Trust Task specs, instead of being invented fresh per demo. **Built**, including bundle-level exclusion: `demo-profiles/` holds the trading-card, Approver and starter profiles, `ACTIVE_DEMO_PROFILE` selects one (or `none`) at runtime, and `none` now also excludes every profile's code from the Metro bundle rather than merely leaving it unregistered — see [`2026-09-10-bm.md`](./reference-app-sdk-packaging/2026-09-10-bm.md).
...
1. **Open Trust Task type→handler registry + a render/approve-deny UI** (needed by #1, #3, #5-human, #8, #9, and useful for #4's stub). **Built.** `TrustTaskRegistry` (`bifold/packages/core/src/modules/trust-tasks/registry.ts`) replaces `ceremony.ts`'s closed if/else dispatch with `registerTrustTask({ typeUri, spec, responseSpec, orchestration, handleRequest, handleResponse })`, resolved via `trustTaskRegistry.resolveForDocumentType(type)`; the pre-existing VRC/witness/discovery branches are themselves now the registry's first four registrations, proving the dispatch is real rather than a parallel mechanism sitting beside the old one. `ITrustTaskDisplayRegistry` (`display/trustTaskDisplayRegistry.ts`) mirrors `ICredentialDisplayRegistry` on the UI side, feeding a generic renderer (`TrustTaskApprovalCard.tsx`). See [`2026-09-06-agent.md`](./reference-app-sdk-packaging/2026-09-06-agent.md). The wallet-side UI was since recast from one Wallet-tab-wide banner to a per-contact surface (`COMPONENT_CONTACT_DETAILS_FOOTER`) plus an app-wide toast listener (`COMPONENT_APP_GLOBAL_LISTENER`), since a real request always names one specific contact — see [`2026-09-10-bm.md`](./reference-app-sdk-packaging/2026-09-10-bm.md).
