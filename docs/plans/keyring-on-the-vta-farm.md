...
| [`2026-09-24-cd.md`](./keyring-on-the-vta-farm/2026-09-24-cd.md) | cd | Brings [`community_vetting_subtask.md`](./keyring-on-the-vta-farm/community_vetting_subtask.md) §2.4's decision to main: option B (Keyring manages the user's VTA; a VTA persona is the member), decided 2026-09-16 and until now recorded only on the unmerged `plan/prague-farm-membership` (`a496e02`, F19/F20). States the persona mechanic as built in 220 (keys borrowed into the wallet's KMS, the card kept from the inline `allow`), and leaves two questions for Alberto: where the card lives under §2.6, and whether borrowed keys may persist |
