...
**Subtasks:** [`community_vetting_subtask.md`](./keyring-on-the-vta-farm/community_vetting_subtask.md) — F2 and F4 for the ecosystem's peer identity vetting ceremony, developed against a local VTI stack until a Farm can host it. [`tsp_rev3_subtask.md`](./keyring-on-the-vta-farm/tsp_rev3_subtask.md) — the TSP Rev 2 → Rev 3 cutover: pack one revision, read two. Parented here because the Prague path forces the question; `openvtc-integration-plan.md` keeps ownership of TSP as a transport. [`own_agent_subtask.md`](./keyring-on-the-vta-farm/own_agent_subtask.md) — F2 for a person with no computer: create and own an agent from the phone, protect the owner key, add a backup owner, and the Farm QR ask. [`agent_policy_and_vault_subtask.md`](./keyring-on-the-vta-farm/agent_policy_and_vault_subtask.md) — two F3 owner features: approval rules the agent keeps, and community credentials kept in the agent's vault and read back by a new or restored phone.
...
3. **Which authority does the join task use?** [[DEV-GUIDE]] gives `https://trusttasks.org/openvtc/vtc/join-requests/submit/1.0`, while [`pnm_cnm_subtask.md`](./openvtc-integration-plan/pnm_cnm_subtask.md) §3.4 records the `trusttasks.org/openvtc/` authority as retired for the PNM catalogue in favour of the canonical `trusttasks.org/spec/` one. Both cannot be current for the same client. Blocks L2's wire work. **Ask upstream now**; the answer is a sentence and the discovery cost at implementation time is a day. **Answered** in [`2026-09-15-al.md`](./keyring-on-the-vta-farm/2026-09-15-al.md) F2: the canonical `spec/vtc/join-requests/submit/0.2`.
4. **Does the Farm accept a hardware-held key, and can a second device be added?** The Admin DID box should be indifferent to where the private key lives, and one grant per device is the obvious model — but neither is stated anywhere, and both are load-bearing for a wallet. **Ours to measure — F2.** Carried by [`own_agent_subtask.md`](./keyring-on-the-vta-farm/own_agent_subtask.md) as measurements M1–M4; a second device is an `acl/grant` from the owner phone (its §4).
...
| [`2026-09-25-cd.md`](./keyring-on-the-vta-farm/2026-09-25-cd.md) | CD | Opens [`own_agent_subtask.md`](./keyring-on-the-vta-farm/own_agent_subtask.md): what the plugin, the VTA and Keyring's code say about owning an agent from the phone; why the plugin's VTA-minted onboarding is not adopted; why the minimum's owner key stays a software Ed25519 key (a Secure Enclave key cannot carry the DIDComm/TSP session), with a hardware owner over REST conditional on measurement; qualifies §3.2's hardware-custody reading on this path accordingly |
| [`2026-09-26-cd.md`](./keyring-on-the-vta-farm/2026-09-26-cd.md) | CD | Opens [`agent_policy_and_vault_subtask.md`](./keyring-on-the-vta-farm/agent_policy_and_vault_subtask.md). Read at the pins: approval rules are one policy row written by an unrestricted admin, with Rego the VTA re-derives byte for byte; enforcement is off by default and invisible to clients; there is no pending-approvals list. The vault is written by whoever holds the credential, and persona-addressed credentials reach the phone, not the VTA, so the phone writes through with the persona's context; proof-set cards are refused (VTI-44 (e)). Real membership and role credentials are DTG credentials with no name. Raises VTI-Q33–Q36 |
