...
*Living plan for the OpenVTC infrastructure-compatibility workstream, at **Rev 25**. Supporting research (TSP/DIDComm learning notes, engineering brief) is kept out of the repo for now; ask if you want it.*
...
- [`openvtc-integration-plan/vta-carriage_subtask.md`](./openvtc-integration-plan/vta-carriage_subtask.md) — VTA-to-VTA carriage for the relationship/witness exchange: how a `vrc/relationship/propose` can be answered by a counterparty's always-on VTA (screened or auto-decided by saved policy while their PNM sleeps) instead of requiring their PNM to be online, while keeping the fully-decentralized PNM-to-PNM path first-class and locality (physical co-presence evidence) strictly optional. The discovery ladder, VTA-side authorization, and the upstream contribution items in both `verifiable-trust-infrastructure` and `openvtc` this requires. Referenced from §7 #6, #9 and extends P4 of `pnm_cnm_subtask.md`.
- [`openvtc-integration-plan/didcomm_v2_subtask.md`](./openvtc-integration-plan/didcomm_v2_subtask.md) — DIDComm v2 in Keyring, §5.4 stage 3 made concrete: a third `Carriage` implementor on Credo's own DIDComm v2 (credo-ts PR #2704, consumed as a pinned npm snapshot after a 0.6.3 → 0.7 hop), the per-connection selection ladder replacing the TSP dev flag, a v2 mode for `mediator-server`, and — conditional on two measured rungs — one Credo transport for the VTI mediator leg. Names the mediator per topology mode. Referenced from §3, §5.4 and §6 Phase D.
...
| 14 (08-18) | [`2026-08-18-al.md`](./openvtc-integration-plan/2026-08-18-al.md) — Alberto. The subtask's §9 step 6 starts in Keyring: propose + issue (shadow) landed and e2e-proven on the prod mediator, with the e2e path stabilized through five diagnosed defects and the suite now gating on the ceremony markers; subtask §4 Layer A corrected to the staged spec's final shapes (digest receipt, `vrc` member, proof REQUIRED, single `notAccepted`) |
| 15 (08-19) | [`2026-08-19-al.md`](./openvtc-integration-plan/2026-08-19-al.md) — Alberto. Step 5's done-when met: presentation assembly of VWC + retained outcome evidence with the verifier's pairing algorithm, live-proven under a self-check gate in the witnessed e2e; three conformance corrections to the shipped witness path (proof-excluding task digest, decoded-byte digest comparison, latest-response terminal pairing); the attended device run (M4) green with the hardware-attestation evidence block proven, surfacing the VWC-sharing gap (the Witnessed badge stays dark under v4 — mechanism undecided) |
| 16 (08-20) | [`2026-08-20-bam.md`](./openvtc-integration-plan/2026-08-20-bam.md) — Brendan. New subplan [`vta-carriage_subtask.md`](./openvtc-integration-plan/vta-carriage_subtask.md): a VTA's always-on signing-oracle-with-policy model ([[VTA-OVERVIEW]], DTTE) is real but has never been wired to the relationship/witness exchange anywhere upstream. An initial pass justified keeping `issue`/witnessing on the PNM by treating witnessing as inherently physical; review caught the conflation — witnessing is already optional and carries no locality requirement of its own, [locality plan](./locality-plan.md) adds that separately — and split the design into three independent axes (negotiation, locality, credential custody) instead of one topology rule. Adjusts `pnm_cnm_subtask.md` §5/§9 and `trust_tasks_subtask.md` §4/§5 additively |
| 17 (08-20) | [`2026-08-20-al.md`](./openvtc-integration-plan/2026-08-20-al.md) — Alberto. **The recast milestone**: step 7 (VWC sharing) built and live-proven, then the whole relationship-and-witness recast (steps 5–7) rehearsed green on attended devices with hardware attestation; the R-Card legacy-trigger starvation fixed; the exchange overlay narrates the dialect; the two debts retired (`@bifold/trust-tasks` platform-neutral plumbing, witness on the real runtime); demo runbook; the R-Card-as-a-task question recorded for the group |
| 18 (08-24) | [`2026-08-24-bam.md`](./openvtc-integration-plan/2026-08-24-bam.md) — Brendan. Part I: the e2e connect failure root-caused with mediator server-side access to **two stacked operational issues** on the shared production `credo-mediator` (a pre-credo-0.6 image whose forward handler silently dropped every message; then a `DirectDelivery` default that doesn't fit this wallet's polling-only pickup) — neither a wallet or protocol bug, both fixed live, suite green. Part II: an investigation into **exchange durability** — the inventory of in-flight state that dies with the JS context, the proposal to fold exchange state out of the document log and advance it on message events rather than timers, the four constraints that bound the benefit (witness-side ephemerality, no background progress, legacy flow not retrofitted, challenge freshness **undecided**), and a pickup-strategy regression on foreground resume found on the way |
| 19 (09-07) | [`2026-09-07-al.md`](./openvtc-integration-plan/2026-09-07-al.md) — Alberto. Review of [`vta-carriage_subtask.md`](./openvtc-integration-plan/vta-carriage_subtask.md) during the pre-Prague consolidation. The three-axis design holds; five things it does not say. Four are blanks rather than disagreements: the plan never names *which* mediator carries a VTA-mediated `propose`; `did:webvh` resolution is absent entirely though `ref-06x` already measured Credo 0.6.3 returning `unsupportedDidMethod` and the adapter is parent Phase D item 3; the policy surface is specified as `pnm-cli` commands the wallet has no equivalent of; and nothing says how a VTA gets attached to a wallet at all. The fifth adds a consideration to §11 Q1 — the OOB-first design's undesigned UX step is largely one we need regardless. Names the contrast demo (`accept` beside `consent`) the plan's own §5 mechanism implies, and why that makes Q3 demo-blocking though not code-blocking |
| 20 (09-08) | [`2026-09-08-al.md`](./openvtc-integration-plan/2026-09-08-al.md) — Alberto. Dispositions on [`vta-carriage_subtask.md`](./openvtc-integration-plan/vta-carriage_subtask.md) §11. Q1 (cold `propose` vs out-of-band pairing) stays **open deliberately** — to be decided against a real deployment, not in the abstract — but its binary framing is rejected: two intermediate positions, a shared-membership gate (proving you hold the recipient's membership credential buys the right to ask, not acceptance) and a VTA-side ACL standing in for pairing, make the answer *pairing first unless something verifiable stands in for it*. Four things need research first, privacy of a pre-relationship membership disclosure among them. Q3 **answered**: the contrast demo is not a Prague target, so Q3 reverts to blocking nothing — superseding the demo-blocking characterisation in Rev 15 **Part II** — review of [`2026-08-24-bam.md`](./openvtc-integration-plan/2026-08-24-bam.md) Part II (exchange durability): adopted in direction. Answers the "doesn't Credo already do this?" question once, on the document's own evidence — it moves continuations into storage that exists rather than inventing persistence. Names the mitigation neither part records, `mediatorPollingInterval: 1_000`, as a *mitigation not a fix* whose safety is conditional on the pickup queue being postgres — a deployment constraint currently living only in a source comment. Adds an evidence caveat: a transport failure on an instrumented physical device is not evidence of a wallet defect until reproduced on a minimally-instrumented one |
| 21 (09-02) | [`2026-09-02-bam.md`](./openvtc-integration-plan/2026-09-02-bam.md) — Brendan (the agent, working independently). **Phase D's core deliverable built and tested**, not just its scope: the `VidResolver` port (`ref-11`) and `direct.ts`'s `pack`/`unpack` (`ref-12`, reusing the real published `@openvtc/vti-tsp-js`'s own CESR/envelope code) complete `tsp-core`'s three ports; `@bifold/trust-tasks` gains the port types and envelope orchestration as a `tsp` namespace; a new `bifold/packages/credo-tsp-adapter` package holds the Askar-backed implementations; a new `TspCarriage` carries real TSP envelopes over the existing DIDComm-v1 connection, dev-flag-gated in `ceremony.ts` and toggleable from a new Developer-settings switch. Also corrects the stage-4 scope below (wallet-to-wallet needs no ecosystem counterparty) and fixes a real bug `ref-12` caught: a `KeyAgreement` derived from an independent key resolves to the wrong public key once a real `VidResolver` is involved. A `yarn e2e:vrc:tsp` spec is written but unverified — no device/emulator was available this session |
| 22 (09-03) | [`2026-09-03-bam.md`](./openvtc-integration-plan/2026-09-03-bam.md) — Brendan (the agent, working independently, plus two direct user course-corrections). `yarn e2e:vrc:tsp` verified clean: a real Askar bug found and fixed (`Buffer.prototype.slice()` yields a view, not a copy, so a sliced peer public key crossed into `Key.fromPublicBytes` as the whole surrounding message and was rejected as "Invalid key data"). User-directed follow-on, ahead of §4.2's stated later-phase ordering: the witness itself now also accepts Trust Task documents over a TSP envelope (`WitnessTaskSessions.ts` dual-carriage), because `sendTrustTaskDocument` auto-selects carriage from the wallet's own flag with no regard for whether the witness can read it — turning on wallet TSP carriage silently broke the witness ceremony until this shipped. New `yarn e2e:vrc:witnessed:tsp:android-only` passes clean on two physical Android devices, both carriage directions, after fixing three more real harness bugs (Metro cache staleness, a cold-boot race, a real device dropping one of 11 required taps in the developer-mode gesture) and one defensive fix (a real notification covering the app). §4.2 and `trust_tasks_subtask.md` §9 step 5 updated to match |
| 23 (09-04) | [`2026-09-04-bam.md`](./openvtc-integration-plan/2026-09-04-bam.md) — Brendan (the agent, working independently). Reprioritized subtask §9 step 4 toward user-facing value: `ref-07-credo-adapter`'s reference-adapter fixture item, previously recorded as blocked, is corrected — `ref-09`'s `KeyAgreement.agree()` port (already shipped, PR #27) resolved `ae14e04`'s stated blocker the same session it was found, so that item is a small unblocked cross-check, not a design question. Deprioritized behind it: the wallet itself now answers `credential-exchange/query` — `ceremony.ts`'s existing dispatch table gains a handler, a new `CredentialExchangeQueryModal` consent prompt (mirroring `RelationshipProposalModal`), and a signed `credential-exchange/present` reply — the happy path, unit-tested (10/10 suites, 78/78 tests) and, same day, device-verified clean on real hardware (`yarn e2e:credential-exchange-query`, Pixel 8 emulator + physical phone), after finding and fixing six real bugs including a genuine upstream `@credo-ts/core` `DcqlService` bug (patched via `.yarn/patches/`); PR #31 opened against #27 |
| 24 (09-13) | [`2026-09-13-al.md`](./openvtc-integration-plan/2026-09-13-al.md) — Alberto. Part I: `main` after the Prague consolidation. Part II: **Credo has DIDComm v2** — unreleased, in [credo-ts#2704](https://github.com/openwallet-foundation/credo-ts/pull/2704) (genaris, 91 commits, `pr-2704` npm snapshots on 0.7.x), which Verana Labs already ships against with patches that turn out to be Data Integrity fixes, not v2 fixes. §3's "Credo has no v2" and §5.4 stage 3's "`vti-didcomm-js` beside Credo" become conditional positions; new subplan [`didcomm_v2_subtask.md`](./openvtc-integration-plan/didcomm_v2_subtask.md) with a Node-only pre-Prague V0 and the 0.7 hop after it |
| 25 (09-16) | [`2026-09-16-bam.md`](./openvtc-integration-plan/2026-09-16-bam.md) — Brendan (the agent, working independently). Fix session for `feat/credo-0.7`'s code review (keyring-wallet/keyring-bifold PR #54): nine findings closed, three of them (`identityFromDid`'s keyAgreement fallback, `checkV2ThreadCorrelation`'s getter-vs-raw-header bug, and a security fix binding DIDComm v2 first-contact identity to a cryptographically-verified key instead of an unauthenticated plaintext claim) squarely in this subtask's `credo-tsp-adapter`/`trust-tasks`/`DidCommV2Carriage` code. The security fix's residual scope (DID-resolution trust, no replay guard) is recorded honestly rather than claimed closed |
...
- **DIDComm**: v1 only in released Credo (0.6.3 and 0.7.0 alike); Aries-style mediator. The VTI mediator is DIDComm v2 + TSP. Credo's own v2 exists unreleased in [credo-ts#2704](https://github.com/openwallet-foundation/credo-ts/pull/2704), consumable as a pinned npm snapshot on 0.7.x; [`didcomm_v2_subtask.md`](./openvtc-integration-plan/didcomm_v2_subtask.md) adopts it for both legs: its Stage A rungs (2026-09-13) showed Credo's envelopes and `vti-didcomm-js`'s are mutually readable and a Credo agent can log into and exchange through the Affinidi mediator, so Keyring↔VTA rides Credo through one custom transport, with OpenVTC's JS library used only for `did:webvh` resolution and message builders.
...
Convergence of the legacy stack (VRC-over-TSP, witness ops as Trust Tasks) is prototyped and live-proven on real devices, dev-flag-gated — not the default posture. Wallet-to-wallet Trust Task documents can carry a real TSP envelope over the existing DIDComm-v1 connection (`TspCarriage`, 2026-09-02), and the witness itself accepts the same documents over that carriage too (`WitnessTaskSessions.ts` dual-carriage, 2026-09-03) — necessary, not optional, once a wallet's TSP flag is on, since carriage selection is a wallet-side decision the witness has no say in. DIDComm-v1 binding 0.2 remains what a fresh install actually uses, and no ecosystem counterparty (`vta-service`, `openvtc` CLI, another implementer) exists for any of this yet. Reasoning: [`2026-09-03-bam.md`](./openvtc-integration-plan/2026-09-03-bam.md).
...
- **`tsp-core`'s role lives in `@bifold/trust-tasks`** (its `tsp` namespace: the `SigningKey`/`KeyAgreement`/`VidResolver` port types and the HPKE-Auth `pack`/`unpack` orchestration) rather than a fourth bifold package — step 1's resolution, applied consistently rather than standing up a duplicate task/port spine. **Zero Credo/Askar/RN imports** there, so the same code runs in Node and RN. `bifold/packages/credo-tsp-adapter` (built, 2026-09-02) holds the concrete Askar-backed port implementations beside `witness-server`/`vrc-*`, consumed via `portal:`. `vti-client` (the PNM/CNM-facing package, DI-exposed via a `TOKENS` entry + the `AgentBridge` pattern) remains unbuilt. `@bifold/trust-tasks`'s `tsp` module and `credo-tsp-adapter` are npm-publishable later if the ecosystem wants them.
...
| **3. Trust Tasks over DIDComm v2** | Credo's own v2 (credo-ts#2704 snapshot) on both legs: our mediator in v2 mode for wallet-to-wallet, one custom Credo transport for the VTI mediator (`@openvtc/vti-didcomm-js` supplies only `did:webvh` resolution and plaintext builders) | the same task documents, unchanged | **Built and proven on physical devices** — [`didcomm_v2_subtask.md`](./openvtc-integration-plan/didcomm_v2_subtask.md): Stage A (ref-15…18) green 2026-09-13, including the binding-0.2 envelope byte-identical to the v1 carriage, v2 mediation on real sockets, and a Credo agent through the Affinidi mediator; V1 (Credo 0.7 hop) and V2 (the carriage) green on simulators 2026-09-14/15, then a witnessed exchange on an iPhone 16 + iPad A16 (2026-09-15) with Coordinate Mediation 2.0 on both wallets. Remaining: B8's mixed-fleet check, and V3 (VTI transport). Uncommitted, in the `keyring-credo-0.7` worktree. See below |
| **4. Trust Tasks over TSP** | `tsp-core`'s Carriage port | the same task documents, unchanged | Split by counterparty, see below — **wallet-to-wallet built and device-proven** (`TspCarriage`, `@bifold/credo-tsp-adapter`, `2026-09-02-bam.md`): over v1 on emulators, and over a DIDComm v2 connection on an iPhone 16 + iPad A16 (2026-09-15) — every Trust Task document a TSP envelope, hardware-attested and witnessed shields on both contacts; ecosystem interop is gated on `vta-service` shipping TSP enabled, which it does not at Cypress ([`pnm_cnm_subtask.md`](./openvtc-integration-plan/pnm_cnm_subtask.md) §2.3) |
...
**Started** (Keyring, `feat/trust-tasks-over-didcomm-v1`): the port extraction hit a real design question before any Credo code was written. Production `@openvtc/vti-tsp-js` (pinned `89d70c4`, 0.2.0) already runs entirely on the noble backend this same effort contributed (§4.4 below, `ref-03-noble-crypto`) — `hpke-js` is dev-only now, never called at runtime — but `pack`/`unpack` and the crypto layer underneath (`crypto/hpke-noble.ts`, `crypto/sign.ts`) still take a raw private key handed over directly, with no entry point for a pre-computed DH result, so an Askar-backed identity (private key never exported) cannot drive any of them as-is. Resolved for the `SigningKey`/`KeyAgreement` half: [`tsp-reference/ref-09-tsp-core-ports`](../../tsp-reference/ref-09-tsp-core-ports/) ports the two static-key DH call sites in a local copy of that same noble construction onto a `KeyAgreement.agree()` operation, proven byte-identical to the unported implementation, still RFC 9180-vector-correct, and satisfiable by a fully opaque, async-only identity (the shape a real Askar RPC would have) — 13/13 checks — and stages the same change as an additive `PR-CANDIDATE.md` against the real upstream files, since this is a natural follow-up to a PR this effort already got merged rather than a cold-start contribution. Reasoning, including a correction (an earlier pass checked a stale local clone and misattributed which library enforces the raw-key requirement): [`openvtc-integration-plan/2026-09-02-bam.md`](./openvtc-integration-plan/2026-09-02-bam.md). Not yet attempted: a real Askar-backed adapter satisfying these ports (only a raw key and a simulated opaque identity have been tried), the `VidResolver` port, the `eddsa-jcs-2022` auth signer, and the Credo trust-task client below — the estimate still stands for that remaining scope.

| Rung | Script does | You learn | Est. |
|---|---|---|---|
| **ref-07-credo-adapter** | Extract the ports (`SigningKey`/`KeyAgreement`/`VidResolver`) from the Phase A–B work; keep the raw-key implementation as the **reference adapter**; build `credo-tsp-adapter` on two `@credo-ts/node` 0.6.3 agents (Askar `Key.fromKeyExchange` + `signMessage`, VIDs via Credo's dids API); run the **same fixture suite against both adapters**; add the `eddsa-jcs-2022` signer for `auth/authenticate` (§4.5); build the **Credo trust-task client for `bindings/didcomm-v1/0.1`** (§7.9) — carriage already proven by ref-06v1/ref-06v1b, so the work here is the module itself (own send path over the Carriage port; the chat API is content-only) — *not* `binding/didcomm/0.1`, which is DIDComm v2.1 and unreachable from Credo 0.6.3; the v2.1 leg, where needed, rides `@openvtc/vti-didcomm-js` beside Credo. **`SigningKey`/`KeyAgreement` started as `ref-09-tsp-core-ports` — see above; this row's own number is stale, see `tsp-reference/README.md`** | The custody boundary made concrete (what stays in Askar, what exits as `secretBytes`); proof the ports don't leak; Credo integration settled by construction | 2–3 wk |
...
| 13 | **A cold-contact Trust Task class on the VTA** — `vta-service`'s authorization pipeline has exactly one non-ACL carve-out today (the ceremony set), and it still requires the sender DID to be pre-registered. Nothing lets a genuinely unenrolled DID reach any handler. A `vrc/relationship/propose/0.1` handler needs a new, narrowly-scoped branch authorizing from the document's own Data-Integrity proof (the VIC pattern, `vtc-service`'s own invitation-verification code), landing in the Policy Decision Point rather than auto-executing, plus a sensible default disposition and a rate limit | PR to verifiable-trust-infrastructure | Without it, a VRC exchange can never run VTA-to-VTA — every unenrolled proposer is refused before any policy gets a say. Detailed in [`vta-carriage_subtask.md`](./openvtc-integration-plan/vta-carriage_subtask.md) §5, §7 |
