...
- [`openvtc-integration-plan/embedded-vta_subtask.md`](./openvtc-integration-plan/embedded-vta_subtask.md) — operating without a hosted VTA, from the phone's own side: bringing VTA-grade policy evaluation (the [[DTTE]]/PDP concept `vta-carriage_subtask.md` names) on-device, woken by a push-triggered mediator fetch rather than an inbound listener. A different topology from both siblings above — `pnm_cnm_subtask.md` is a client of a remote VTA, `vta-carriage_subtask.md`'s VTA-mediated mode is also remote/hosted — not a competing plan for the same problem.
...
| 24 (09-14) | [`2026-09-14-bam.md`](./openvtc-integration-plan/2026-09-14-bam.md) — Brendan. New subtask [`embedded-vta_subtask.md`](./openvtc-integration-plan/embedded-vta_subtask.md): on-device VTA-grade policy evaluation, distinct from both `pnm_cnm_subtask.md`'s and `vta-carriage_subtask.md`'s remote/hosted-VTA topologies. Drafted 09-04, updated on this date after a 240-commit pull surfaced `vta-carriage_subtask.md` (adopts its PDP/DTTE vocabulary; clarifies decentralized negotiation already needs no VTA, narrowing this subtask's actual scope to the policy layer) and `keyring-on-the-vta-farm.md` (corrects the pin/tripwire baseline); also corrects two stale findings (`credo-tsp-adapter` and `@bifold/trust-tasks`'s `carriage.ts` are now real, shipped work) while confirming the push-wake gap it identified is still unaddressed |
