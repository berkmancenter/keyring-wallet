...
*Living plan for the OpenVTC infrastructure-compatibility workstream, at **Rev 13**. Supporting research (TSP/DIDComm learning notes, engineering brief) is kept out of the repo for now; ask if you want it.*
...
| 13 (08-19) | [`2026-08-19-al.md`](./openvtc-integration-plan/2026-08-19-al.md) — Alberto. Step 5's done-when met: presentation assembly of VWC + retained outcome evidence with the verifier's pairing algorithm, live-proven under a self-check gate in the witnessed e2e; three conformance corrections to the shipped witness path (proof-excluding task digest, decoded-byte digest comparison, latest-response terminal pairing); the attended device run (M4) green with the hardware-attestation evidence block proven, surfacing the VWC-sharing gap (the Witnessed badge stays dark under v4 — mechanism undecided) |
