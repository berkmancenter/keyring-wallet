...
*Rev 2 (07-22) added: Cypress-state verification, RN crypto deep-dive, Docker/local-VTA analysis, Trust Tasks explainer, contribution roadmap. Rev 3 (07-27): re-audited upstream after an 82-commit week and reworked the roadmap into a **reference-script ladder** (§6). Rev 4 (07-29): merged the team-review adjustments that survived an evidence check against upstream code — the `tsp-core` / `credo-tsp-adapter` package split, external-package-not-fork, and two-adapter conformance testing (adopted); static-until-rotation envelope selection and per-transport proof rules (rejected — contradicted by spec + shipping code; see review notes in [`2026-07-29-bam.md`](./openvtc-integration-plan/2026-07-29-bam.md) + fact-check record). Also: Cypress is at RC-0, and REST auth now requires `eddsa-jcs-2022`-signed documents (§4.5).*
*Rev 5 (08-05): applied the six findings of [`2026-08-05-bam.md`](./openvtc-integration-plan/2026-08-05-bam.md) — **§5.2 task model now depends on a `Carriage` port, not on the TSP wire** (F1); **§7.9 + ref-07 re-scoped to a DIDComm-v1 binding we author**, since `binding/didcomm/0.1` is v2.1 and unreachable from Credo 0.6.3 (F2); **new §4.2a task-layer coexistence policy** — N/N−1 acceptance, because upstream's clean-cutover cadence does not survive an app-store release cycle (F3); §8 non-negotiables scoped to the TSP work (F4); **new §8.1 VWC outcome-evidence retention**, surfaced but still uncosted (F5); §7.6's route and timing corrected — the witnessed-exchange spec is now a **normative dependency of the VWC** and has been **invited by the framework editor as a joint appendix**, so authoring moves parallel with Phases A–C rather than post-ladder (F6). Detail and sequencing: [`2026-08-05-trust_tasks_subtask.md`](./openvtc-integration-plan/2026-08-05-trust_tasks_subtask.md).*
...
*Added 2026-08-05, review Finding 3. §4.2 above governs **envelope formats**; this governs **task versions**. The two are independent — an app can be perfectly TSP-capable and still be bricked by a Type URI that was deleted.*
...
**The task model depends on a `Carriage` port, not on the TSP wire** (revised 2026-08-05, review Finding 1). Earlier revisions wired this `TT --> WIRE --> IFACE`, which made the trust-task layer depend on envelope orchestration — contradicting §2.3's own claim that the task layer "rides any transport" and that everything built there is "transport-portable by construction", and [Trust Tasks SPEC §1.1 goal 1](https://github.com/trustoverip/dtgwg-trust-tasks-tf/blob/main/SPEC.md#11-design-goals) normatively. Concretely, that coupling would have dragged HPKE, CESR and the noble backend into three places that need none of them:
...
> **Moved out of the ecosystem phase 2026-08-05.** The witnessed-exchange Trust Task specification is **no longer post-ladder, and no longer a private-authority route**. Two things changed: DTG Core Credentials makes a qualifying Trust Task specification a *normative dependency* of the VWC's `taskContext` binding, and the framework editor has **invited** the specification as a joint appendix ([#173](https://github.com/trustoverip/dtgwg-trust-tasks-tf/issues/173#issuecomment-5189686547)). Spec **authoring** now runs in parallel with Phases A–C (document work, long registry lead time, and the editor's timeline is not ours); spec **implementation** lands at Phase D/E. See §7.6, §8.1, and [`2026-08-05-trust_tasks_subtask.md`](./openvtc-integration-plan/2026-08-05-trust_tasks_subtask.md) §9.3 for the full order.
...
- **Non-negotiable at every level**: VRC module tests stay green; witness flow untouched *by the TSP transport work* — changes to it are gated on §7.6 and on `e2e:vrc:devices` staying green (see review Finding 4); `e2e:vrc:devices` remains the attestation proof; legacy DIDComm v1 stack fully supported throughout.

### 8.1 Unscoped work item: VWC outcome-evidence retention

*Surfaced 2026-08-05 from the [Trust Tasks subtask](./openvtc-integration-plan/2026-08-05-trust_tasks_subtask.md) §4 Layer C. Not currently costed in any phase of §6.*
...
- Companion review of this plan: [`2026-08-05-bam.md`](./openvtc-integration-plan/2026-08-05-bam.md) — four proposed adjustments (`tsp-core` dependency direction; §7.9's DIDComm-v1-vs-v2.1 binding commitment; task-layer coexistence policy; §8 non-negotiables vs §7.6) · subtask detail: [`2026-08-05-trust_tasks_subtask.md`](./openvtc-integration-plan/2026-08-05-trust_tasks_subtask.md)
...
- Team review: [`2026-07-29-bam.md`](./openvtc-integration-plan/2026-07-29-bam.md) (Brendan) — five proposed adjustments; adopted after evidence check: package split, no-fork, two-adapter conformance; rejected: static-until-rotation selection, per-transport proof, `TspMessaging` service name (upstream uses `TSPTransport`)
