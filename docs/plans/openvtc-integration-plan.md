...
**Open, and blocked on nobody.** Whether VRC and VWC move to `bbs-2023`; whether the VWC carries `digest` or a commitment; whether our session challenge adopts the canonical transcript. All three are ours to decide, and all three are cheaper to decide before the witness specification freezes (subtask §9 step 2) than after.
...
| **ref-06v1c-task-layer** ✅ | The task layer on the proven carriage: `vtc/relationships/request/0.1` (the spec upstream migrated its own VRC exchange onto) processed by `@openvtc/trust-tasks` 0.6.0's `consumeInbound` §7.2 pipeline between two Credo agents — request/`#response` both directions, the `trust-task-error`+`inResponseTo` decline idiom, the §4.8.1 cross-check with `theirDid` as transport-authenticated sender, §8.1 oracle suppression, and the binding's "case 2" probed (Credo: distinct "No connection associated" failure). 13 checks green | The machinery our `vrc/*`/`witness/*` specs will run on, working end-to-end on the app's stack; response-side consumption works via `RESPONSE_SPEC` (the #173 gap is guidance, not code); `proof: REQUIRED` enforcement is real (proof *verification* deliberately stubbed — DI-suite work is Phase D) | done (08-11) |
| **ref-06v1d-carrier** | The carrier question answered empirically: the identical Trust Task exchange built twice — basic-message + `~attach` (as drafted) vs. a **dedicated message `@type`** via a minimal Credo module — measuring module code, chat-record pollution, and what a Trust-Task-*unaware* agent does on receiving each (harmless bubble vs. problem-report vs. silence) | The evidence for the binding's ⚠ open §2 question ("is `basic-message` the right carrier at all?") — the time-sensitive review the framework editor asked of us; decides whether Keyring's chat surface carries protocol traffic | 1–2 d |
| **ref-06w-witnessed-exchange** | The two-thread witnessed exchange as **draft** task documents over the chosen carriage: `vrc/relationship/propose` → `witness/session` (child thread, `parentThreadId`) → `witness/session/submit` → `vrc/relationship/issue` both directions — asserting `taskContext` = the session document's `id`, per-variant proof declarations, and `#response` retention as evidence | Whether the qualifying profile is satisfiable by a real witnessed exchange — the exact question the joint-appendix invitation exists to test; prices the outcome-evidence retention (review A5) as a measured cost; the appendix specs get authored from a running exchange, not prose. Pure framework 0.4 — no ceremony machinery; a later half-step can add `ceremony{enactment, step, prev}` members as the layer's first wire contact | 2–3 d |
