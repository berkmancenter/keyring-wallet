...
**Started** (Keyring, `feat/trust-tasks-over-didcomm-v1`): the port extraction hit a real design question before any Credo code was written — `vti-tsp-js`'s `pack`/`unpack` and `hpke-js`'s `CipherSuite` both require a raw private key handed over directly, with no entry point for a pre-computed DH result, so an Askar-backed identity (private key never exported) cannot drive either as-is. Resolved for the `SigningKey`/`KeyAgreement` half: [`tsp-reference/ref-09-tsp-core-ports`](../../tsp-reference/ref-09-tsp-core-ports/) ports the two static-key DH call sites in a copy of `ref-03`'s already-vector-verified raw-`@noble` HPKE-Auth onto a `KeyAgreement.agree()` operation, proven byte-identical to the unported implementation, still RFC 9180-vector-correct, and satisfiable by a fully opaque, async-only identity (the shape a real Askar RPC would have) — 13/13 checks. Reasoning: [`openvtc-integration-plan/2026-09-02-bam.md`](./openvtc-integration-plan/2026-09-02-bam.md). Not yet attempted: a real Askar-backed adapter satisfying these ports (only a raw key and a simulated opaque identity have been tried), the `VidResolver` port, the `eddsa-jcs-2022` auth signer, and the Credo trust-task client below — the estimate still stands for that remaining scope.
