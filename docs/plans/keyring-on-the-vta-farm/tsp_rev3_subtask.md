...
**Blocked on:** R4.
**Standing:** done for the send-only introduction upstream's services use. `packInviteRev3` builds `XRFI` over the custody ports and is byte-identical to the package's deterministic invite; one invite per peer per session precedes the first Trust Task, routed via our mediator with a `Reply_Path` of `[mediator, us]`; a received `XRFA` is acknowledged and released rather than refused. The accept (`XRFA`) and cancel (`XRFD`) frames are packed too, byte-identical to the package's (bifold `609ff45`). Against upstream's own state machine, `tsp-reference/ref-04s-rev3-relationship` shows the VTA answering our invite with an `XRFA` that names it (signature verified) and taking our `XRFD`; its §7.3 answering cancel is refused upstream (VTI-38 in the findings doc). Still open, and ours: the app sends neither — no peer in the ecosystem legs invites us, and ending a relationship belongs with *Leave community*, which the UI/UX plan's U1 builds and which needs the invite's digest kept past the session that sent it. The invite race cannot arise against a service that never initiates, and is left to the package's `resolveInviteRace`.
...
**Standing:** green in the lab for the ecosystem legs. The carriage is chosen from the peer's published `TSPTransport` (§4.2 of the parent plan), and the vetting ceremony passes on two devices with phone ↔ VTA (manager key backing both TSP ports) and phone ↔ VTC on Rev 3, against `vta-service` and `vtc-service` built with TSP and a mediator built `--features tsp`. A message above 12,285 bytes through the mediator is measured by `tsp-reference/ref-04r-rev3-long-frame`: long-form frames (`0xFB`) up to 61,182 bytes on the wire, Direct and Routed, delivered byte-identical and verified. The switch to the long form comes a little past 12,285 bytes of frame, since the count does not cover all of it (12,327 bytes was still short). Open: the applicant ↔ vetter leg (persona documents advertise no `TSPTransport`, VTI-Q11) and the same run on the Farm.
