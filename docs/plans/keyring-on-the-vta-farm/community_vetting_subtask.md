...
Create the implementation branch on a checkpoint of the Credo 0.7 / DIDComm v2 line; add and advance pins (§4).

**Done when:** `yarn lint`, `yarn typecheck`, `yarn test` and `bifold/packages/core` `yarn test` pass on the checkpoint before anything is added; `sync-external.mjs` reports the new pins with reasons.
**Blocked on:** nothing.
