...
**Status:** In execution on `feat/prague-farm-membership` (both repositories, pushed; wallet `ae42501`, bifold `7fd7798`). Reviewed in [#53](https://github.com/berkmancenter/keyring-wallet/pull/53).
...
**Reasoning:** `[2026-09-15-al.md](./2026-09-15-al.md)` — the measurements behind §2–§3, and which positions of the parent plan they supersede; `[2026-09-16-al.md](./2026-09-16-al.md)` — what the first two days of execution measured, the positions it changed (§2.1, §2.4, §2.5, §3.2, §5) and the open decision in §2.4; `[2026-09-16-bm.md](./2026-09-16-bm.md)` — the state boundary of §2.6 and the rendering rules of §3.9, and what each of them turns on in §2.4's open decision. `[2026-09-24-cd.md](./2026-09-24-cd.md)` — §2.4 as decided (option B, 2026-09-16), with the persona mechanic that F20 measured and the two points it leaves open. Upstream-facing findings, numbered and versioned: `[docs/VTI_UPSTREAM_FINDINGS.md](https://github.com/berkmancenter/keyring-wallet/blob/feat/prague-farm-membership/docs/VTI_UPSTREAM_FINDINGS.md)`. This document states current design only; see `[CLAUDE.md](../CLAUDE.md)`.
...
### 2.4 Keys — Keyring manages its own VTA, and the VTA's persona is the member

Every Keyring user has a Personal VTA, and Keyring is its manager, as upstream's TUI and browser plugin are. The member of a community is a **persona** of that VTA, not the phone. Upstream's join guide documents the same (`vti-setup` `developer/03-joining-a-community.md`):

- **The VTA mints and holds the member identity.** A persona is a `did:webvh` the VTA mints on a DID-hosting server it is registered with, one per community, never the VTA's primary DID ([[VETTING-DESIGN]] fixes the join persona at the start of an application). The VTA keeps the persona's keys.
- **Keyring runs the ceremony as the persona.** The VTA has no join task of its own. The client submits `manifest/0.2` and `submit/0.2` under the persona, as upstream's reference client does: openvtc records a persona's keys as references, fetches the secret with `keys/export-secret/0.1` when it must act, and signs and seals locally. Its join test states that submission *"never touches the VTA"*. Keyring does the same. `VtaClient.ensurePersona` mints the persona, borrows its key-agreement and signing keys into the wallet's KMS over the manager session, and records the persona with those KMS ids so later joins reuse it. Keyring's Credo transport then addresses the community directly.
- **Keyring reaches its VTA over DIDComm v2 through the VTA's mediator, or over the VTA's REST with a DID-signed login**, the two transports a VTA advertises. Never DIDComm v1.
- **The card arrives inline in the community's `allow`** (`verdict.with.vmc`, and `verdict.with.roleVec` when a role is granted), and Keyring keeps it in `VtiCommunityStore`.

The phone-minted `did:peer:2` member that P4 first built survives only as the proof of the transport and the screens; P4b replaced it with the persona. The phone's own manager `did:key` is software Ed25519. Its hardware target is chosen by measurement among a P-256 `did:key` in the Secure Enclave (if the VTA accepts one), StrongBox Ed25519 (Android API 33+), or software Ed25519 wrapped by a hardware key. The parent's custody position (§3.2) is unchanged; this subtask sequences it. §3.6 records why TSP Rev 3 eases the hardware case, and [`tsp_rev3_subtask.md`](./tsp_rev3_subtask.md) §2.4 is where the hardware-custody choice is revisited now that Rev 3 is what the wallet sends on the peer leg.
...
**This is the written case for §2.4, and it binds because the VTA is the member.** No local mirror of anything the VTA owns: personas and faces, contexts, the join persona, the legal-name face §7 already places there. Those surfaces render what the current session's replies contain. Which privileged surfaces render is decided by a who-am-I reply, not by anything recorded at enrolment, and `vta/app-state/*` (§5) is where progress that must outlive an install belongs.

**Binding whoever holds the keys.** The VTC owns membership, the published requirements and the verdict, so none of those is ever rendered from a last-known value (§7). And Keyring keeps durable local state for three things it owns, each load-bearing:

- **Its own keys and credentials** — the Askar store, the VRC/witness exchange, and the credentials it holds including the Vetting Statements it receives. The in-person VRC exchange has no VTA in its path, and the argument for `eddsa-jcs-2022` there is that a credential formed in person verifies with no `@context` resolution ([`openvtc-integration-plan.md`](../openvtc-integration-plan.md) §4.5). A wallet that cannot render its own credentials without a reachable counterparty fails precisely where that path is meant to work.
- **A member identity that survives a restart.** Already measured: the VTC delivered both credentials to the phone and they sat at the mediator *"for a DID the app no longer remembers because it mints a new `did:peer:2` on every connect"* (`[2026-09-16-al.md](./2026-09-16-al.md)` F14), which is why P5's first rung is one identity kept across restarts. It is now the manager's identity, and it must persist.
...
The boundary, stated once so §7's screens can be checked against it: **no local mirror of state a counterparty owns; durable local state only for what Keyring owns.** §2.4 says which state is the VTA's.
---
...
**S10:** the disclosure preview is produced by the same code that will sign and send the card, not by a screen that re-renders the face beside it. The ecosystem already has the task for it: `vta-sdk` at the baseline registers `persona/disclosure/preview/1.0` beside `persona/disclosure/present/1.0` (`vta-sdk/src/trust_tasks.rs`), and `pnm-core` 0.9.1 implements both (`dist/persona/disclosure.js`), so §3.3's *"disclosure preview, then presentation"* is two calls and S10 renders the response to the first. (`vta/contexts/preview-delete/1.0` is upstream's same pattern for a destructive operation.)

Two rules that do not depend on where the keys live:
...
- **The agent's three stores — a port if adopted.** Beside the vault (secrets and credentials) and agent memory, `vta-sdk` registers `vta/app-state/*` at the baseline: *"versioned, namespaced, per-context JSON an application owns and the VTA does not interpret,"* addressed `(contextId, namespace, key)` and gated on context access. Its shape is worth knowing before anything is designed around it: `put` with `expectedVersion` (`0` = create-only) or an RFC 7386 `mergePatch`, `get`, `get-many` (≤256), `put-many` (≤64, `independent` or `atomic`), `list` with `sinceVersion` as a change feed that always includes tombstones, `delete` leaving a versioned tombstone, and a failed precondition that returns the VTA's current version **and value** so a conflict resolves without a re-read. Upstream records why this is not `vta/memory/*`: *"'forget everything' has to stay a safe thing to ask an agent, which it cannot be if account state lives there."* `pnm-core` 0.9.1 implements the client (`dist/app-state/`), but §2.1 makes that package a reference rather than a dependency — so this is a port onto our own transport, sized by those six operations.
