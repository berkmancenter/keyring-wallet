...
**Status:** In execution on `feat/ui-ux` (both repositories). Linking by QR and without a QR, the ticket QR and the agent screen are built; the QR link is green on a physical iPad. Simulator runs come first and real devices are the final check (§6). Target: the live demo at OSS Summit Europe, Prague, 1 October 2026.
...
3. **The lab enrolment page, experimented locally.** A small page beside the lab stack that adds the QR half upstream's form lacks — steps 1–4 of the journey above. It is where the demo links its phones. It is not a product: it exists to prove the exchange end to end (§6, U8). A real phone reaches it only over **HTTPS**: iOS App Transport Security lets the app use plain HTTP to `localhost` alone, so the page's public URL is a tunnel, never a LAN address.
...
- The vetter's ticket renders as a QR with its link twin under it (*Copy link*), and, in dev builds, the hidden link text the VRC screens already expose for the harness. Every QR has a fixed maximum size (240 pt): the shared renderer defaults to the screen's width, which on an iPad produces a code too large for another phone's camera frame.
