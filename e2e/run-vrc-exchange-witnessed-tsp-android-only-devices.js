 ...
 * with the wallet-to-wallet Trust Task documents (discovery/propose/issue)
 * carried over the real TSP envelope stack instead of the default DIDComm-v1
 * binding. Otherwise identical to
 * run-vrc-exchange-witnessed-android-only-devices.js — same witness, same
 * hardware attestation, same VWC issuance. Flow lives in
 * lib/witnessedExchangeFlow.js (`useTspCarriage: true`) — see that file's
 ...
 * (witnessed-vrc-manager.ts) — and is unaffected by this flag; only the
 * peer-to-peer VRC exchange itself moves onto TSP. See
 * docs/plans/openvtc-integration-plan/2026-09-02-bam.md for the TSP scope
 * correction this rests on (wallet-to-wallet only, not an ecosystem-interop
 * claim).
 *
 ...
 *
 * Usage:
 *   node run-vrc-exchange-witnessed-tsp-android-only-devices.js
 *   (or: yarn e2e:vrc:witnessed:tsp:android-only from repo root)
 *
...
await runWitnessedExchange({
  ...
  name: "vrc-exchange:witnessed:tsp:android-only",
  useTspCarriage: true,
});
