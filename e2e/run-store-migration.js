/**
 * Askar 0.2→0.6 store-migration E2E (Phase 3 gate):
 *
 ...
 *      against the NEW iOS build so the old-format store holds a real credential.
...
const PEER = { firstName: "Alice", lastName: "Anderson" }; // iOS (new build)
...
try {
  ...
  console.log(`[e2e] installing BASELINE apk: ${BASELINE_APK}`);
  try {
    adb(`uninstall ${APP_ID}`);
  } catch {
    /* not installed */
  }
  adb(`install -t "${BASELINE_APK}"`);
  ...
  try {
    ...
    await ios.terminateApp(APP_ID).catch(() => {});
    await sleep(3000);
    await ios.activateApp(APP_ID);
    await sleep(3000);
  ...
  await Promise.all([
    ...
    completeOnboarding(ios, PEER),
  ]);

  // New iOS wallet shows the invitation (the baseline build lacks the
  ...
  const invitationUrl = await showRelationshipInvitation(ios);
  ...
  await Promise.all([
    ...
    acceptCredentialOfferFromChat(ios),
  ]);
