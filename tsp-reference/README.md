...
| [`ref-08-credential-exchange`](./ref-08-credential-exchange/) | `credential-exchange/{query,present,pending/*}` (subtask §9 step 4) against a real `vta-service`: the DID-auth handshake round-trips (`@bifold/trust-tasks`'s eddsa-jcs-2022 proof construction verified byte-compatible with the real ecosystem, not assumed), every REST caller needs an ACL role before anything credential-exchange-specific applies, and `credential-exchange/query` is confirmed absent from the REST dispatch table — messaging (DIDComm/TSP)-only in this build, cross-checked directly against `vta-service`'s dispatch macro | a local VTA (`e2e/lib/vta.js`'s `startVta()`) |
