...
| [`ref-07g-outcome-evidence-pairing`](./ref-07g-outcome-evidence-pairing/) | which rule decides that a `taskContext`-bearing credential's exchange **completed**: the same seven exchanges (witness, vetting, a minted thread, a cancellation, a self-signed response, an error, a counterfeit) run through the witness task spec's rule, Keyring's `outcomeEvidence.ts`, cred-spec #18's text, and a generic rule for the Trust Tasks framework. The witness spec's rule does not generalize; **Keyring accepts a control `#response` and a holder-signed `#response` as completion**; **#18 rejects the real witness ceremony**; the generic rule — declared outcome-evidence type, signed by the initiating document's recipient, thread paired through the initiating document — gets all seven right | nothing |
