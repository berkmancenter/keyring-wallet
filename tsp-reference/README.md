...
| [`ref-13-macos-ble-central`](./ref-13-macos-ble-central/) | can a **Mac** be the locality sensor's BLE central, the way the Linux witness is? `witness-server`'s provider is `node-ble`/BlueZ, Linux-only — which makes a Linux box a prerequisite for every locality run and leaves the unimplemented **iOS peripheral** unverifiable on a Mac-only desk. Replays `runTranscriptExchange`'s exact sequence through CoreBluetooth. **Act 1 answered on 2026-09-11: yes** — `poweredOn`, 12 peers discovered in 22s, no entitlement needed. Act 2 (the full transcript exchange) awaits something that advertises a locality EID. Settles the build order: the iOS peripheral can be developed against a Mac | a Mac with Bluetooth |
