...
| [`ref-04f-farm-cross-mediator`](./ref-04f-farm-cross-mediator/) | a community on **another operator's** mediator: a Farm client asks `first-vtc` (storm mediator) for its manifest — DIDComm answers live storm → Farm in ~1 s on three sender routes; TSP is stored but unanswered (VTI-Q15) | network (the public Farm) |
...
| [`ref-07b-statement-shape`](./ref-07b-statement-shape/) | what [cred-spec #45](https://github.com/trustoverip/dtgwg-cred-spec/issues/45)'s `StatementCredential` profile costs an implementer: the VWC re-expressed as a `dtg:witnessed` profile gives **identical verdicts on all four checks a verifier runs**; the registered type alias keeps Keyring's dispatch intact (and exposes a latent bug — `isPeerVrcCredential` files any alias-less statement credential in Contacts); the Trust Task layer reads `credentialSubject` **zero times**; and the artifacts run against **`dtg-credentials` 0.7.0**, the Rust reference implementation, isolate **9 independently fatal divergences** — one of them ours (`issuer` as an object, off WD02), most of them unspecified (WD02 says nothing about unknown members). Fixtures built by the shipped `buildWitnessCredentialJson` | cargo (act 4 only) |
| [`ref-07c-predicate-coverage`](./ref-07c-predicate-coverage/) | #45 open question 2 answered by measurement: a **plain namespaced-string predicate contributes 0 quads** and can be swapped for its own negation with the signature intact; an **unresolved CURIE prefix silently becomes a URI scheme** — signed, tamper-evident, and meaning a *different* IRI to a verifier holding a different context; `object.digestMultibase` is free (`credentials/v2` defines and `@protects` it); profile-defined members are unsigned until someone writes their terms; `eddsa-jcs-2022` closes all of it | nothing |
| [`ref-07d-vocabulary-trust-path`](./ref-07d-vocabulary-trust-path/) | #45 open question 3, reframed: driving the **real shipped `createVrcDocumentLoader`**, an unbundled community vocabulary makes the wallet **fetch that community's server at verification time** (unauthenticated, uncached, no allowlist), is **unverifiable offline**, and lets the vocabulary host **retroactively invalidate every credential** issued under it — and silently un-cover the predicate of every one issued after. The context is the right mechanism; unversioned and fetched-at-verify is not | nothing |
| [`ref-07e-crossimpl-vwc`](./ref-07e-crossimpl-vwc/) | Keyring and **`dtg-credentials` 0.7.0** (the Rust catalogue the VTC mints from) verifying each other's VWCs **in both directions**, with real signatures. Found a bug in **our** `eddsa-jcs-2022`: we attached the document's `@context` to the proof config, which [vc-di-eddsa](https://www.w3.org/TR/vc-di-eddsa/) does only for `rdfc-2022` — the crate was right, both directions pass after one deleted line, and `ref-07`'s fixtures needed re-signing. Also: our digest of a real captured VRC is **byte-identical** to theirs (`zQmWjM6…`), the WD01 member name `digest` is accepted but the `sha256:<hex>` **value** form is not, Keyring's four VWC checks score **2/4** on their credential and **4/4** after the WD02 placement fix, **neither** side's DTG `@context` resolves (both 404 — costing JCS nothing and RDFC everything), and #45's profile test is shown **passing on a credential whose ceremony never happened** | cargo (acts 1-4) |
| [`ref-07f-predicate-registry`](./ref-07f-predicate-registry/) | cred-spec #52's predicate registry, built from its own worked entries, with a verifier configured from the generated `accept-list.json` run against **Keyring's real credentials through Keyring's real code** (`credentialTypes.ts`, `witnessedExchangeContext.ts` executed from source). The format carries object kinds, `taskContext`, member schemas and fail-closed handling, and verification needs no network; for a wallet the accept-list is also the dispatch table. It cannot enforce `subjectObjectRelationship` (free text) or `minimumIssuerScope` (no scope property until #46), the unpublished `witness-context` schema decides whether Keyring's hardware/locality evidence is conformant, and an active-only accept-list makes deprecation retroactively reject issued credentials | nothing |
...
| [`ref-13-jcs-canonicalizer-corpus`](./ref-13-jcs-canonicalizer-corpus/) | T1 from the SDK-packaging plan's remediation phasing: whether `@bifold/trust-tasks`'s `canonicalize`-based JCS and `@openvtc/trust-tasks`'s own `canonicalJson` agree. Corrects the plan's premise first — the installed/pinned `@openvtc/trust-tasks@0.9.0` has **no** `canonicalJson` at all (it shipped starting at 0.13.0), so the "two canonicalizers ship today" risk is not yet live, only pending T0's pin advance. Comparison against 0.16.8's `canonicalJson` (vendored, frozen): **28/28 required cases byte- and digest-identical** across unicode/surrogate pairs, number formatting, deep member ordering, and empty/absent members — plus one non-gating bonus divergence found on `Date`/`toJSON` objects (outside the plan's four categories, and not reachable from any JSON-wire document) | nothing |
