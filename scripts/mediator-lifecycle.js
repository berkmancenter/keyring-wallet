  ...
  await new Promise((resolve, reject) => {
    ...
    srv.once("error", (err) => {
      reject(
        err.code === "EADDRINUSE"
          ? new Error(
              `port ${port} is already in use — stop that process or pass --port <other>`
            )
          : err
      );
    });
...
async function startTunnel(port, readyTimeoutMs = 60000) {
  ...
  proc.kill("SIGINT");
  throw new Error(
    `cloudflared did not produce a tunnel URL within ${readyTimeoutMs}ms`
  );
}
...
function ensureEnvFile() {
  ...
  if (!existsSync(envSamplePath))
    throw new Error(`neither ${envPath} nor ${envSamplePath} exists`);
 ...
 */
function writeMediatorUrl(mediatorUrl, mediatorV2Url) {
  ...
  setLine("MEDIATOR_URL", mediatorUrl);
  // Always written, blank when v2 is off: a stale v2 invitation from a
  // previous run is as wrong as a stale v1 one.
  setLine("MEDIATOR_V2_URL", mediatorV2Url);
...
async function startMediator({
  ...
  const invitationV2Path = join(mediatorDir, ".mediator-invitation-v2");
  rmSync(invitationPath, { force: true });
  rmSync(invitationV2Path, { force: true });
  ...
  const env = {
    ...
    MEDIATOR_INVITATION_V2_PATH: invitationV2Path,
    MEDIATOR_DIDCOMM_VERSIONS: didcommV2 ? "v1,v2" : "v1",
    MEDIATOR_VERBOSE: verbose ? "true" : "false",
  };

  if (fresh)
    execSync("yarn fresh", { cwd: mediatorDir, env, stdio: "inherit" });
  ...
  while (Date.now() < deadline) {
    if (exited)
      throw new Error(
        "the mediator exited before publishing an invitation — see its output above"
      );
    if (existsSync(invitationPath)) {
      // The mediator writes the v2 file before the v1 one, so it is complete here.
      const mediatorV2Url = existsSync(invitationV2Path)
        ? readFileSync(invitationV2Path, "utf8").trim()
        : undefined;
      return {
        mediatorUrl: readFileSync(invitationPath, "utf8").trim(),
        mediatorV2Url,
        publicUrl,
        stop,
        process: proc,
